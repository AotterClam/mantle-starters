...
const coreRoot = resolve(root, "blank/node_modules/@aotter/mantle");
const expected = [
...
for (const [dir, name, sourcePath] of expected) {
  ...
  if (!existsSync(corePath)) failures.push(`installed Core skill is missing: ${corePath}`);
  else assertSame(agentPath, corePath);
}
